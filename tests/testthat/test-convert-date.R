...
test_that("convert.Date() | warning test", {
    shush(expect_message(convert(as.Date("2000-01-01"), "logical",
                                 quiet = FALSE),
                         "'x' cannot be converted to 'logical'."))

    shush(expect_message(convert(as.Date("2000-01-01"), "integer",
                                 quiet = FALSE),
                         "'x' was converted to total of days since"))
    shush(expect_message(convert(as.Date("2000-01-01"), "double",
                                 quiet = FALSE),
                         "'x' was converted to total of days since"))
    shush(expect_message(convert(as.Date("2000-01-01"), "numeric",
                                 quiet = FALSE),
                         "'x' was converted to total of days since"))

    shush(expect_message(convert(as.Date("2000-01-01"), "duration",
                                 quiet = FALSE),
                         "There's no time to convert."))
    shush(expect_message(convert(as.Date("2000-01-01"), "period",
                                 quiet = FALSE),
                         "There's no time to convert."))
    shush(expect_message(convert(as.Date("2000-01-01"), "difftime",
                                 quiet = FALSE),
                         "There's no time to convert."))
    shush(expect_message(convert(as.Date("2000-01-01"), "hms", quiet = FALSE),
                         "There's no time to convert."))
})
